...
function buildReportHtml(result, lang = 'tr') {
    ...
    // Header logosu — email-client uyumlu (Outlook/Gmail/Zimbra/Apple Mail).
    // Inline CSS + table layout; gradient destekleyen client'lar görsel olarak
    // gösterir, desteklemeyenlerde solid fallback (#1e3a8a) ile MailTrustAI
    // markalaması okunabilir kalır.
    const reportTitle = isTR
        ? `${escapeHtml((companyName || 'MailTrustAI').toUpperCase())} GÜVENLİK RAPORU`
        : `${escapeHtml((companyName || 'MailTrustAI').toUpperCase())} SECURITY REPORT`;
